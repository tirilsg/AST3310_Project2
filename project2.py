...
class EnergyTransportation:
    ...
    def run_simulation(self,init,p=0.01):
        ...
        while init.m > 0 and init.r > 0:
            ...
            m_values = np.append(m_values,init.m)
            r_values = np.append(r_values,init.r)
            rho_values = np.append(rho_values,init.rho)
            P_values = np.append(P_values,init.P)
            L_values = np.append(L_values,init.L)
            T_values = np.append(T_values,init.T)
            F_con_values = np.append(F_con_values,init.F_con)
            F_rad_values = np.append(F_rad_values,init.F_rad)
            Q_total = np.append(Q_total,init.energies.tot)
            Q_PP1_values = np.append(Q_PP1_values,init.energies.Q_PP1)
            Q_PP2_values = np.append(Q_PP2_values,init.energies.Q_PP2)
            Q_PP3_values = np.append(Q_PP3_values,init.energies.Q_PP3)
            Q_CNO_values = np.append(Q_CNO_values,init.energies.Q_CNO)
            ...
            init.step(dm) 
            if L_values[-1]>0.995*L_values[0] and init.L<0.995*L_values[0]:
    ...
    def opacity_sanity(self, tol):
        ...
        log_10_T=np.array([3.750, 3.755, 3.755, 3.755, 3.755, 3.770, 3.780, 3.795,  
                           3.770, 3.775, 3.780, 3.795, 3.800])
        log_10_R=np.array([-6.00, -5.95, -5.80, -5.70, -5.55, -5.95, -5.95 , -5.95,
