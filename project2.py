...
class EnergyTransportation:
    ...
    def run_simulation(self,init,p=0.01):
        ...
        while init.m > 0 and init.r > 0:
            ...
            init.step(dm)
            #calculations of core and convection sizes
            if L_values[-1]>0.995*L_values[0]: #if the most recent calculated value of L is larger than the initial scaled value of L
                self.core_size=init.r/r_values[0] #the core size variable is redefined by the most recent radius, scaled by the total radius of the star
            if F_con_values[-1]!=0 and init.F_con==0 and variable==False: #if we havenn't already defined the convection zone
                self.conv_zone=((r_values[0]-init.r)/r_values[0]) #the convection zone size is redefined as the interval between the surface and the current radius, scaled
                variable= True #we've defined the variable conv_zone, and set the varable equal to True
            if init.r==r_values[-1]: #if the new radius equals the most recently appended radius
                break #we break and allows us to exit the while-loop 
